...
public class Execute extends Executable {

	public Execute(IInternalContest inContest, IInternalController inController, Run run, RunFiles runFiles) {
...
//		runs[0].setStatus(edu.csus.ecs.pc2.core.model.Run.RunStates.REJUDGE);
		...
		Execute execute = new Execute(serverConnection.getIInternalContest(), serverConnection.getIInternalController(), runs[0], runFiles);
		...
		IFileViewer iFileViewer = execute.execute(true);
